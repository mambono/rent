...
function update_bikes_table()
{
    ...
    $.getJSON( "<?= URL::to('/') ?>/bookings/my", function( data ) {
...
function populate_bikes_table(data)
{
    ...
    else
    {
		...
		html  += '<table id="bikes-table" class="table-bordered datatable" width="100%" cellspacing="0" style="background-color:#ffffff;"> <thead> <tr> ';
         html += '<th width="15%">Bikes Name </th>'; 
        html += '<th width="5%">Color </th>';  
        html += '<th width="15%">Location </th>'; 
        html += '<th width="25%">Booked Dates</th>';  
        html += '<th width="12%">Total Hours</th>'; 
        html += '<th width="10%">Cost </th>';
        html += '<th width="18%">Action </th>';  
        html += '</tr> <tbody>';
        ...
        $.each( data, function( key, val )
        {
            ...
            html += '<tr class="item" tabletype="bikes" recordid="' + val.id +'" id="' + val.id +'">'
            ...
            html += '<td><a href="<?= URL::to('/') ?>/bookings/delete/' + val.id + '" bikes_editid="' + val.id + '" class="btn btn-danger bikes-edit-link">Delete</a> ';   
            html += '&nbsp;&nbsp;<a href="https://www.paypal.com/bike=' + val.id + '&cost=' + val.totalcost + '" target="_blank" bikes_editid="' + val.id + '" class="btn btn-success bikes-edit-link">Pay via Paypal</a> '; 			
            html += '</tr>';
        });
